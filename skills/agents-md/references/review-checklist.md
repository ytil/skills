...
- **HOW** — can a fresh agent **install, run, build, test, and verify** from this file alone? Walk that list explicitly; a test/QA gate is present far more often than "how to actually start the app," which is the usual gap. Commands should be copy-pasteable and confirmed against the repo (run them if you can). Weight **verify** highest: agents now work long unwatched stretches, so what the file says about proving a change works — the health check, the seed script, the smoke test, what "done" means here — is the leg that pays off most.
...
- **Generic behavior steering.** Flag "be concise," "don't over-engineer," "always verify your work," "think step by step," "ask before destructive actions," "don't add features beyond what was asked." **Fix:** delete the generic half and keep the project-specific half — not "confirm before risky commands" but "`pnpm deploy:staging` posts to the live #releases channel; ask first."
