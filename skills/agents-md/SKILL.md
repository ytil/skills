...
2. **Cover WHAT, WHY, HOW, BOUNDARIES.** _What_ = stack + a map of where things live. _Why_ = what the major pieces are for, so the agent grasps intent. _How_ = the real commands to build, test, run, and **verify** — name what proves a change actually works here; agents work long unwatched stretches now, so this leg pays off most. _Boundaries_ = what's unsafe to do unattended **in this repo specifically** — not the generic "ask before risky actions" the harness already supplies (#11).
