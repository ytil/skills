...
5. **Use progressive disclosure.** Push task- or area-specific detail out of the always-loaded file into `@`-imported topic docs, path-scoped `.claude/rules/`, or skills — then point to them so the agent reads them only when relevant. The dividing line: **facts the agent holds constantly go in the file; procedures (deploy/release checklists) go in skills.**
