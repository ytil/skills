...
import { parseArgs, resolveSynthesizer, UsageError, USAGE } from "./lib/args.mjs";
import { analysisPrompt, reviewPrompt, synthesisPrompt } from "./lib/prompts.mjs";
...
function ensureDeps() {
  ...
  const res = spawnSync("npm", ["install", "--no-fund", "--no-audit", "--loglevel=error"], {
    cwd: SCRIPTS_DIR,
    stdio: ["ignore", "inherit", "inherit"]
  });
  if (res.status !== 0) {
    fail(`npm install failed in ${SCRIPTS_DIR} — install dependencies manually and retry`, 2);
  }
}
...
async function main() {
  ...
  if (!fs.existsSync(config.cwd)) fail(`--cwd does not exist: ${config.cwd}`, 1);
  ...
  const { runClaude, preflightClaude, validateClaudeEffort } = await import(
    "./lib/claude-agent.mjs"
  );
  ...
  const reportDir = config.out ? path.resolve(config.out) : defaultReportDir(config.task);

  const finishReport = () => {
    const report = buildReport({ task: config.task, config, synthesizer, artifacts, timings, failures });
    const meta = {
      task: config.task,
      cwd: config.cwd,
  ...
  const [resA, resB] = await Promise.allSettled([
    timedCall("analysis A (claude)", timings, "analysisA", () =>
      runClaude({ prompt: taskPrompt, role: config.claude, cwd: config.cwd, timeoutMs })
    ),
    timedCall("analysis B (codex)", timings, "analysisB", () =>
      runCodex({ prompt: taskPrompt, role: config.codex, cwd: config.cwd, timeoutMs })
    )
  ]);
  if (resA.status === "fulfilled") artifacts.analysisA = resA.value;
  else failures.push({ stage: "analysis A (claude)", error: resA.reason.message });
  if (resB.status === "fulfilled") artifacts.analysisB = resB.value;
  else failures.push({ stage: "analysis B (codex)", error: resB.reason.message });
  ...
  else failures.push({ stage: "review of A (codex)", error: revA.reason.message });
  if (revB.status === "fulfilled") artifacts.reviewOfB = revB.value;
  else failures.push({ stage: "review of B (claude)", error: revB.reason.message });
  ...
  try {
    artifacts.verdict = await timedCall("synthesis", timings, "verdict", () => {
      const run = synthesizer.side === "claude" ? runClaude : runCodex;
      return run({ prompt: synthPrompt, role: synthesizer, cwd: config.cwd, timeoutMs });
    });
  } catch (err) {
    failures.push({ stage: "synthesis", error: err.message });
    const reportPath = finishReport();
    fail(`synthesis failed — no verdict. Partial report with all artifacts: ${reportPath}`, 3);
  }
  ...
  if (config.json) {
    process.stdout.write(
      JSON.stringify({ verdict: artifacts.verdict, reportPath, timings, failures }, null, 2) + "\n"
    );
  } else {
    process.stdout.write(`${artifacts.verdict}\n\n---\nFull report: ${reportPath}\n`);
  }
}
