...
export function parseArgs(argv) {
    ...
    for (let i = 0; i < argv.length; i++) {
        ...
        if (arg === "--help" || arg === "-h") {
            ...
            config.claude = parseRoleSpec(
                "--claude",
                arg.slice("--claude=".length),
            );
        } else if (arg.startsWith("--codex=")) {
            config.codex = parseRoleSpec(
                "--codex",
                arg.slice("--codex=".length),
            );
        } else if (arg.startsWith("--synthesizer=")) {
            config.synthesizer = parseSynthSpec(
                arg.slice("--synthesizer=".length),
            );
        } else if (
            arg === "--claude" ||
            arg === "--codex" ||
            ...
            throw new UsageError(
                `${arg} requires the full form ${arg}=<...> (see --help)`,
            );
            ...
            const { value, consumed } = takeValue(
                arg,
                argv[i + 1],
                "--timeout",
            );
            ...
            if (!Number.isFinite(n) || n <= 0) {
                throw new UsageError(
                    `--timeout must be a positive number of minutes, got "${value}"`,
                );
            }
            config.timeoutMin = n;
            i += consumed;
            ...
            if (config.task !== null) {
                throw new UsageError(
                    "task given twice — quote the task text as a single argument",
                );
            }
            config.task = arg;
        }
